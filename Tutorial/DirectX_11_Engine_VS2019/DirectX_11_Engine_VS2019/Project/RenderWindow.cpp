...
bool RenderWindow::Initialize(WindowContainer* pWindowContainer, HINSTANCE hInstance, std::string window_title, std::string window_class, int width, int height)
{
	...
	this->window_class = std::move(window_class);
	...
	{
		ErrorLogger::Log(static_cast<HRESULT>(GetLastError()), "CreateWindowEX Failed for window: " + this->window_title);
		return false;
	}
...
LRESULT CALLBACK HandleMessageSetup(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_NCCREATE:
	{
		...
		auto pWindow = static_cast<WindowContainer*>(pCreate->lpCreateParams);
